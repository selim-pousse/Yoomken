...
function initBoxy() {
	/* affiche la barre de defilement du bas */
	$('.boxy-inner').css({'overflow-y': 'auto','overflow-x': 'auto'});
	objBoxy  =  Boxy.get($('#smartfilter-boxy'));
    	...
    	<?foreach ($parts_or as $or): $parts_and = explode('AND',$or);?>
    		<?foreach ($parts_and as $and): $filt = $filters[$and];?>
    				addFilter(i,befortd,noOr,befortr,i==1?'':(isor?'OU':'ET')
    							...
    							,'<?=$filt->value?>','<?=($filt->label)?>','<?=$filt->attach?>');
    				befortd = i++;
    		<?endforeach;?>
    		isor = true;
    		befortr = noOr++;
	...
	objBoxy.center('y');objBoxy.center('x');
};
			    ...
			    <ul style='list-style-type: none;'>
			        ...
			        <li>
			            ...
			            <?foreach ($filtersdef as $key => $val) :?>
			                <option value="<?=$key?>" related="gabarit-op-<?=$key?>"><?='Filtrer par '.gettext('_6_'.$key)?></option>
			            <? endforeach;?>
			            </select>
...
<textarea class='hide' id="gabarit-op-<?=$key?>" >
	...
	<? foreach ($filt as $opin) :?>
          <option value="<?=$opin->op?>" related="<?=$opin->in?>"><?=($operators[$opin->op]->alias)?></option>
    <? endforeach;?>
    </select>
</textarea>
<? endforeach;?>
...
<textarea style="display:none" id="gabarit-in-<?=$in->id?>">
	...
	<?endif;?>
	<input id='input-txt-{0}' type='text' value="" name='dyna_txt-{0}' placeholder='' title=''/>
...
<form id="smartfilter_form" class="" method="POST" action="/smartfilter/save" accept-charset="utf-8">
	...
	<div style='width:260px;'>
      	...
      	<input class='infieldlabel enabled' placeholder='<?=_('LABEL_FILTER')?>' type='text' value='<?=$filter->alias?>' name='alias' title='<?=_('LABEL_FILTER')?>' style='width:270px;'/><br/>
     	<input type='hidden' value='<?=$filter->id?>' name='idfilter'/>
     	<input type='hidden' value='<?=$ftype?>' name='type'/> 
    </div>
                
    <fieldset>

        <legend style='font-weight: bold'><?=_('RULE_FILTER')?></legend>
