...
class MySQL_User extends BaseUser {
    ...
    public static function authenticate($username, $password = '') {
        ...
        $auth_sql = "SELECT * FROM " . MYSQL_PREFIX_VIEW . "users
	...
	public static function logout($id=null)
	{
		...
		$updatell_sql = "UPDATE `user` SET last_activity_usr = DATE_SUB( NOW( ) , INTERVAL 20 MINUTE)  WHERE pid_usr=:id";
    ...
    public static function get($id) {
        ...
        $find_sql = "SELECT * FROM " . MYSQL_PREFIX_VIEW . "users
    ...
    public function status($s=-1, $message=null) {
    	if ($this->status)
    	{
        	$s = $this->status->s;
        ...
        if($s>-1) $this->status->is($s, $message);
        //} else {
            return $this->status;
        //}
    }
    ...
    public function ip($ip=null) {
        if($ip) {
            ...
            if($this->user_id && $this->autosave) {
                $updateip_sql = "UPDATE " . MYSQL_PREFIX_VIEW . "users SET last_known_ip=:ip WHERE user_id=:id";
            ...
            if($this->ip) {
                ...
                $getip_sql = "SELECT last_known_ip FROM " . MYSQL_PREFIX_VIEW . "users WHERE user_id=:id";
    ...
    public function lastLogin($ll=null) {
        if($ll) {
            ...
            if($this->user_id && $this->autosave) {
                //$updatell_sql = "UPDATE " . MYSQL_PREFIX_VIEW . "users SET last_login=FROM_UNIXTIME(:ll) WHERE user_id=:id";
                $updatell_sql = "UPDATE `user` SET last_activity_usr=NOW() WHERE pid_usr=:id";
                $updatell = self::$db->prepare($updatell_sql);
				//_debug($updatell_sql);
                $updatell->execute(array(
                    ':id' => $this->user_id
            ...
            if($this->last_login) {
                ...
                $getll_sql = "SELECT last_login FROM " . MYSQL_PREFIX_VIEW . "users WHERE user_id=:id";
...
class MySQL_Status extends BaseStatus {
    ...
    public static function of($user) {
        ...
        $status_sql = "SELECT status as s, message, s.user_id FROM " . MYSQL_PREFIX_VIEW . "status as s ";
        //if(is_int($user)) {
            $status_sql .= "WHERE user_id = :user";
        //} else {
        //    $status_sql .= "LEFT JOIN " . MYSQL_PREFIX_VIEW . "users as u ON u.user_id=s.user_id WHERE u.username = :user";
        //}
        $status_sql .= " LIMIT 1";
        ...
        return new Status($status->user_id,$status->s, $status->message);
    }

    public function is($s, $message='') {
        /*$status_sql = "INSERT INTO " . MYSQL_PREFIX_VIEW . "status (user_id, message, status) VALUES(:user_id, :message, :s)
        ...
        $changed_status = ($s!=$this->s);
		//_debug($changed_status);
        if($changed_status) {        
            // broadcast status update to friends
            /*$update_friends_sql = "INSERT INTO " . MYSQL_PREFIX_IM . "messages (from_id, to_id, message, type)
                SELECT :user_id AS from_id, friend_id as to_id, :status as message, 's' as type FROM " . MYSQL_PREFIX_VIEW . "friends as friends
                LEFT JOIN " . MYSQL_PREFIX_VIEW . "status as status ON friends.friend_id = status.user_id WHERE friends.user_id = :user_id AND status.status > 0";
            */
            $update_friends_sql = "INSERT INTO " . MYSQL_PREFIX_IM . "messages (from_id, to_id, message, type)
                SELECT :user_id AS from_id, friends.user_id as to_id, :status as message, 's' as type FROM " . MYSQL_PREFIX_VIEW . "friends as friends
                LEFT JOIN " . MYSQL_PREFIX_VIEW . "status as status ON friends.user_id = status.user_id WHERE friends.friend_id = :user_id AND status.status > 0";
            
            $update_friends = self::$db->prepare($update_friends_sql);
            //_debug($update_friends_sql);
            $update_friends->execute(array(
                ':user_id' => $this->user_id,
                ':status' => $this->s . ':' . $this->message
            ));
        }
		
		/*$updatell_sql = "UPDATE `user` SET online_usr = :s WHERE pid_usr = :id";
        $updatell = self::$db->prepare($updatell_sql);
        $updatell->execute(array(
            ':s' => $s,
            ':id' => $this->user_id
...
class MySQL_Message extends BaseMessage {
    ...
    public static function send($from=0, $to=0, $message='') {
        ...
        $send_sql = "INSERT INTO " . MYSQL_PREFIX_IM . "messages (from_id, to_id, type, message)
    ...
    public static function get($id) {
        ...
        $get_sql = "SELECT * FROM " . MYSQL_PREFIX_IM . "messages WHERE message_id=:id LIMIT 1";
    ...
    public static function getMany($from_or_to, $user) {
        ...
        $get_sql = "SELECT message_id, u1.username as `to`, u2.username as `from`, message, `type`, sent FROM " . MYSQL_PREFIX_IM . "messages as m
                    LEFT JOIN " . MYSQL_PREFIX_VIEW . "users as u1 ON m.to_id = u1.user_id
                    LEFT JOIN " . MYSQL_PREFIX_VIEW . "users as u2 ON m.from_id = u2.user_id WHERE ";
        if(is_numeric($user)) {
        ...
        if($get_messages->rowCount()) {
            ...
            $mark_read_sql = "UPDATE " . MYSQL_PREFIX_IM . "messages SET `read` = 1 WHERE message_id IN(" . join(',', $message_ids) . ")";
    ...
    public function sent($ts=null) {
        if($ts) {
            ...
            if($this->id && $this->autosave) {            
                $updatets_sql = "UPDATE " . MYSQL_PREFIX_IM . "messages SET sent=FROM_UNIXTIME(:ts) WHERE message_id=:id";
            ...
            if($this->sent) {
                ...
                $getts_sql = "SELECT sent FROM " . MYSQL_PREFIX_IM . "messages WHERE message_id=:id LIMIT 1";
    ...
    public function from($from=null) {
        ...
        if($from) {
            ...
            if($this->id && $this->autosave) {
                $updatefrom_sql = "UPDATE " . MYSQL_PREFIX_IM . "messages SET from=:from WHERE message_id=:id";
            ...
            if($this->from) {
                ...
                $getfrom_sql = "SELECT from FROM " . MYSQL_PREFIX_IM . "messages WHERE message_id=:id LIMIT 1";
    ...
    public function to($to=null) {
        ...
        if($to) {
            ...
            if($this->id && $this->autosave) {
                $updateto_sql = "UPDATE " . MYSQL_PREFIX_IM . "messages SET to=:to WHERE message_id=:id";
            ...
            if($this->to) {
                ...
                $getto_sql = "SELECT to FROM " . MYSQL_PREFIX_IM . "messages WHERE message_id=:id LIMIT 1";
    ...
    public function message($message=null) {
        if($message) {
            ...
            if($this->id && $this->autosave) {
                $updatemsg_sql = "UPDATE " . MYSQL_PREFIX_IM . "messages SET message=:message WHERE message_id=:id";
            ...
            if($this->message) {
                ...
                $getmsg_sql = "SELECT message FROM " . MYSQL_PREFIX_IM . "messages WHERE message_id=:id LIMIT 1";
    ...
    public function save($id=null) {
        ...
        if($id) {
            $savemsg_sql = "UPDATE " . MYSQL_PREFIX_IM . "messages SET from_id=:from, to_id=:to, type=:type, message=:message, sent=:sent WHERE message_id=:id";
            ...
            $savemsg_sql = "INSERT INTO " . MYSQL_PREFIX_IM . "messages (from_id, to_id, type, message, sent) VALUES (:from, :to, :type, :message, :sent)
...
class MySQL_Friend extends BaseFriend {
    ...
    public static function of($user=0, $offline=false) {
        ...
        $friends_of_sql = "SELECT users.username as u, status.status as s, groups.name as g FROM " . MYSQL_PREFIX_VIEW . "friends as friends
            LEFT JOIN " . MYSQL_PREFIX_VIEW . "users as users ON friends.friend_id = users.user_id
            LEFT JOIN " . MYSQL_PREFIX_VIEW . "status as status ON users.user_id = status.user_id
            LEFT JOIN " . MYSQL_PREFIX_VIEW . "groups as groups ON friends.group_id = groups.group_id
    ...
    public static function get($id) {
        ...
        $friend_get_sql = "SELECT users.username as u, status.status as s, groups.name as g FROM " . MYSQL_PREFIX_VIEW . "friends as friends
            LEFT JOIN " . MYSQL_PREFIX_VIEW . "users as users ON friends.friend_id = users.user_id
            LEFT JOIN " . MYSQL_PREFIX_VIEW . "status as status ON users.user_id = status.user_id
            LEFT JOIN " . MYSQL_PREFIX_VIEW . "groups as groups ON friends.group_id = groups.group_id
    ...
    public static function find($friend, $user=0) {
        ...
        $friend_find_sql = "SELECT users.username as u, status.status as s, groups.name as g FROM " . MYSQL_PREFIX_VIEW . "friends as friends
            LEFT JOIN " . MYSQL_PREFIX_VIEW . "users as users ON friends.friend_id = users.user_id
            LEFT JOIN " . MYSQL_PREFIX_VIEW . "status as status ON users.user_id = status.user_id
            LEFT JOIN " . MYSQL_PREFIX_VIEW . "groups as groups ON friends.group_id = groups.group_id ";
        ...
        if($group) {
            ...
            if($this->id && $this->autosave) {
                if(!is_int($this->group)) {
                    $group_id_sql = "INSERT INTO " . MYSQL_PREFIX_VIEW . "groups (name) VALUES (:group)
                ...
                $updategroup_sql = "UPDATE " . MYSQL_PREFIX_VIEW . "friends SET group_id=:group_id WHERE id=:id";
            ...
            if($this->group) {
                ...
                $getgroup_sql = "SELECT group FROM " . MYSQL_PREFIX_VIEW . "friends WHERE id=:id LIMIT 1";
    ...
    public function save($id=null) {
        ...
        if($id) {
            $savefriend_sql = "UPDATE " . MYSQL_PREFIX_VIEW . "friends SET user_id=:user, friend_id=:friend, group_id=:group WHERE id=:id";
            ...
            $savefriend_sql = "INSERT INTO " . MYSQL_PREFIX_VIEW . "friends (user_id, friend_id, group_id) VALUES (:user, :friend, :group)
