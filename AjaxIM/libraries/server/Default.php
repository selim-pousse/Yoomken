...
class Default_IM extends IM {
    ...
    function __construct() {
		...
		//require_once ('/system/lib/cookie.class.php');
		//require_once ('/system/lib/session.class.php');
		
        //session_start();
        $ses = Session::getInstance();
        if($ses->isLogged()) {
            $this->username = $ses->me()->url;
            $this->user_id = $ses->me()->pid;
			// convert the yoomken status to im status
			if ($ses->me()->status == 1 || $ses->me()->status == 2) $this->s = 1;
			else if ($ses->me()->status == 3) $this->s = 2;
			else $this->s = 0;	
		}
    ...
    public function autologin() {
        //if($user = User::authenticate($this->username)) {
       	if($user = User::get($this->user_id)) {
            ...
            return array('r' => 'logged in', 's' => session_id(),'u'=> $user->username, 'f' => $online);
        } else {
            return array('r' => 'error', 'e' => 'not logged');
        }
    }
    ...
    private function _comet() {
		...
		$usr = User::get($this->user_id);
        while(true) {
        	if(connection_aborted()) {
            	$user_status = new Status($this->user_id);
    			$user_status->is(0, 'offline');
            	return;
			}
			$usr->status();
