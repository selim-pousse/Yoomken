...
abstract class BaseFacebook
{
  ...
  public function getSignedRequest() {
    if (!$this->signedRequest) {
      if ($this->allowSignedRequest && (!empty($_REQUEST['signed_request']) || !empty($_REQUEST['signedRequest'])) ) {
        $this->signedRequest = $this->parseSignedRequest(
          empty($_REQUEST['signed_request'])? $_REQUEST['signedRequest']:$_REQUEST['signed_request']
        );
  ...
  public function getLoginUrl($params=array()) {
    ...
    return $this->getUrl(
      ...
      array_merge(
        array(
          'client_id' => $this->getAppId(),
          'redirect_uri' => 'http://'.$_SERVER["HTTP_HOST"] . '/app.php?go=user/login_fb', // possibly overwritten
          'state' => $this->state,
  ...
  /*protected function getCode() {
  	$server_info = array_merge($_GET, $_POST, $_COOKIE,$_REQUEST);
