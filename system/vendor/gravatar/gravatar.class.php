...
class Gravatar {
	...
	protected $client;
	protected $email = "selim.pousse@gmail.com";
	protected $pass = "asbalik81";
	...
	public function __construct($pass = "", $email = "") {
		...
		if ($email) $this->email = strtolower(trim($email));
	}
	...
	protected function client() {
		...
		if ($this->client === null) {
			$this->client = new XML_RPC_Client('/xmlrpc?user='.md5($this->email), 'secure.gravatar.com');
		}
	 ...
	 */
	public function saveData($data, $rating = 0, $slurp = true) {
	 ...
	 * @returns mixed boolean(false) on failure, image string on success
	 */
	public function saveUrl($url, $rating = 0) {
