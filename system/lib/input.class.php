 ...
 * Manifest
 * 
 * system/lib/input.class.php
 * 
 * Class to manage security when receiving information
 * It is based on Code Igniter's CI_Input class
 * 
...
class Input 
{
	...
	function __construct()
	{
		...
		$this->use_xss_clean	= ($this->registry->global_xss_filtering === TRUE) ? TRUE : FALSE;
		$this->allow_get_array	= ($this->registry->enable_query_strings === TRUE) ? TRUE : FALSE;
	...
	*/
	 ...
	 * @access public
	 */
	public function escape($string,$type=false)
	{
		if (is_string($string))
		{
			if($type=="json")
			{
				...
				{
					$elmt=mysql_real_escape_string(strip_tags_attributes($elmt,array("<param>","<embed>","<iframe>","<object>","<br>","<a>")));
				}
				$string = mysql_real_escape_string(json_encode($json,JSON_FORCE_OBJECT));
			}
			else if($type=="rich")
			{
				$string = strip_tags($string,"<br /><br/><br><a>");
				
			}
			else  $string = addslashes(htmlspecialchars($string));
	...
	{	
		return ucfirst( strtolower( trim( preg_replace('/\s+/',' ', $str) ) ) );
	}
	...
	*/
	...
	{
		$charset = $this->registry->get('charset');
