...
$.GrowingInput = function(element, options){
    ...
    var resize = function(){
        lastValue = value;
        value = element.val();
        if (value == '') value = options.title;
        
        if(options.scroll && value != options.title)
        {
        	var box = element.parent().prev();
        	//console.log(box);
        	if (box.length)
        	{
	            if (box.offset().top != element.offset().top)
	            {
	        		$("#mainsearch .textboxlist-bits").scrollTo(element,{ axis:'y', offset:-2});
	        	}
        	}
        }
    ...
    this.resize = resize;
    this.setOptions = function (opt){ options = $.extend(options, opt); return self;};
    init();
};
