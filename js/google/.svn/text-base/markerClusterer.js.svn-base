...
var styles = [[{
  ...
  function refreshMap(limit,searchterm) {
        if (markerClusterer) {
        ...
        var markerImage = new google.maps.MarkerImage(imageUrl,
          ...
          $.getJSON('/ajax/people/map/'+limit+'/search/'+searchterm,function(data){
				if (data.success) 
				{
					...
					for (var i = 0; i < data.meta.nbread; i++) {
				          ...
				          function toggleBounce() {
				          	$.each(markers,function(){this.setAnimation(null)});
					   		...
					   		//debugger;
					        $('#timeline .isotope-post').removeClass('bulle-selected');
					        $('.'+this.data.id+'-usr.isotope-post').addClass('bulle-selected');
  ...
  function initializeMap(curentPosition,myname,limit_result,searchterm) {
  	  lib.map = true;
	  ...
	  userPosition = myLatlng;
	    ...
	    refreshMap(limit_result,searchterm);
  }